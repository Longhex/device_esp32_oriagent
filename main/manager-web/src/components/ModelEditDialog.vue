  ...
  methods: {
              ...
              if (model.configJson) {
                Object.keys(model.configJson).forEach((key) => {
                  if (this.isSensitiveField(key) && model.configJson[key]) {
                    ...
                    model.configJson[key] = `${sensitiveName} của bạn`;
                  }
                });
              }
            }
    ...
    validateJson(value) {
      try {
        ...
        this.$message.error({
          message: 'Phải nhập định dạng từ điển (vd {"key":"value"}), nếu lưu sẽ dùng dữ liệu gốc',
          showClose: true,
        });
        return null;
      } catch (e) {
        this.$message.error({
          message: 'Sai định dạng JSON (vd {"key":"value"}), nếu lưu sẽ dùng dữ liệu gốc',
          showClose: true,
        });
        return null;
      }
    ...
    getSensitiveFieldName(fieldName) {
      const keyMap = {
        api_key: "Khóa API",
        personal_access_token: "Token truy cập cá nhân",
        access_token: "Token truy cập",
        token: "Token",
        secret: "Khóa bí mật",
        access_key_secret: "Khóa truy cập bí mật",
      ...
      if (this.isSensitiveField(field)) {
        ...
        if (!this.form.configJson[field] || this.form.configJson[field].trim() === "") {
          ...
          if (this.originalValues[field]) {
            ...
            this.$set(this.form.configJson, field, `${sensitiveName} của bạn`);
          }
          // 清除临时存储的原始值
