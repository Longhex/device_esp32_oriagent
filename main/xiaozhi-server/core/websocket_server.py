...
def _setup_websockets_logger():
    """Configure all loggers related to websockets and filter invalid handshake errors"""
        ...
        try:
            ...
            try:
                ...
                if hasattr(websocket, "closed") and not websocket.closed:
                    ...
                    # If there's no closed attribute, just try to close
                    await websocket.close()
        ...
        try:
            async with self.config_lock:
                ...
                if new_config is None:
                    ...
                    return False
                self.logger.bind(tag=TAG).info(f"Successfully obtained new configuration")
                # 检查 VAD 和 ASR 类型是否需要更新
                update_vad = check_vad_update(self.config, new_config)
                update_asr = check_asr_update(self.config, new_config)
                self.logger.bind(tag=TAG).info(
                    f"Checking if VAD and ASR types need update: {update_vad} {update_asr}"
                )
                # 更新配置
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If it's a whitelisted device, skip token validation and allow
                return
            else:
