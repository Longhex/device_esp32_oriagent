...
async def handle_mcp_message(
    ...
    """Process MCP messages, including initialization, tool lists, and tool call responses"""
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                if next_cursor:
                    ...
                    # Refresh tool cache, ensure MCP tools are included in the function list
                    ...
                    try:
                        ...
                        if len(json_objects) > 1:
                            ...
                            if merged_dict:
                                ...
                                raise ValueError(f"Failed to parse any valid JSON objects: {args}")
                        else:
                            raise ValueError(f"Argument JSON parsing failed: {args}")
                    except Exception as e:
                        logger.bind(tag=TAG).error(
                            f"Argument JSON parsing failed: {str(e)}, original args: {args}"
                        )
                        raise ValueError(f"Argument JSON parsing failed: {str(e)}")
        elif isinstance(args, dict):
            arguments = args
        else:
            raise ValueError(f"Argument type error, expected string or dict, actual type: {type(args)}")

        # Ensure arguments is a dict type
        if not isinstance(arguments, dict):
            raise ValueError(f"Arguments must be dict type, actual type: {type(arguments)}")
        ...
        if isinstance(raw_result, dict):
            ...
            if isinstance(content, list) and len(content) > 0:
                if isinstance(content[0], dict) and "text" in content[0]:
                    # Directly return text content without JSON parsing
