...
class AuthManager:
    ...
    Generates and verifies authentication triplets: client_id, device_id, and token (HMAC-SHA256).
    The token does not contain plaintext client_id/device_id; it only carries the signature + timestamp.
    client_id/device_id are passed during connection.
